...
public interface UserMapper {
	...
	@Select({ "<script>" + "select *from member " + "<if test='userid != null'> where userid=#{userid}</if>"
			+ "</script>" })
	...
	/* /////////////phy ///////////// */
	@Select({ "<script>" + "select * from member" + "<if test = 'access == 1'>" + " where access = 0" + "</if>"
			+ "<if test = 'access == 2'>" + " where access = 1" + "</if>" + "<if test='userid != null'>"
