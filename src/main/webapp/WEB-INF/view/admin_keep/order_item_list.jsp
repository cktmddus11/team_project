					...
					<div class="dropdown" style="width: 200px; height: 40px;">
						<div class="select" style="padding: 0; padding-left: 10px;">
							<span id="my_ll_b"> <c:if
									test="${empty param.selectvalue}">재고 상태</c:if> <c:if
			...
			<c:forEach var="w" items="${keeplist}">
				<ul class="order_list1">
					...
					<li class="out_num_1" id="my_ll_b"
						style="text-align: center; width: 10vw;">${w.itemnum}</li>
					<li class="item_name_1" id="my_ll_b"
						style="text-align: center; width: 20vw;">${w.itemname}</li>
					<li class="out_num_1" id="my_ll_b"
						style="text-align: center; width: 10vw;">${w.whousingquant}</li>
					<li class="out_num_1" style="text-align: center; width: 15vw;">
						<span id="my_ll_b"> <c:if test="${w.state==0 }">정품</c:if> <c:if
								test="${w.state==1 }">불량</c:if>
					</span>
					</li>
				</ul>
			</c:forEach>
			<div class="list_footer" style="border-top: 1px solid;">
				<div class="page_block">
					...
					<span class="page_nums" style="width: 22vw;">
						<form action="order_item_list.store" method="post" name="f">
							<input type="hidden" name="pageNum" value="1" />
