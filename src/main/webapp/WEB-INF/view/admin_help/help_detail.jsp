...
<body>
	...
	<div class="product-name">
		...
		<div class="input-name">
			<div class="input-text-name">
				<input class="i-t-name" value="${help.boardcode==1?'공지사항':'FAQ'}"
					readonly error="0" align="left" padding="15" name="itemname">
			</div>
		</div>
	</div>
		...
		<div class="input-name">
			<div class="input-text-name">
				<input class="i-t-name" error="0" align="left" padding="15"
					name="itemname"
					value="<c:if test="${help.filter eq '0'}">전체</c:if><c:if test="${help.filter eq '1'}">배송</c:if><c:if test="${help.filter eq '2'}">교환/반품/환불</c:if><c:if test="${help.filter eq '3'}">기타</c:if><c:if test="${help.filter eq '4'}">상품</c:if><c:if test="${help.filter eq '5'}">주문결제</c:if>"
					readonly>
			</div>
		</div>
	</div>
	...
	<div class="product-name">
		...
		<div class="input-name">
			...
			<c:if test="${empty help.qnafileurl}">
				<input class="i-t-name" error="0" align="left" padding="15" readonly
					value="첨부파일 없음">
			</c:if>
		</div>
	</div>
