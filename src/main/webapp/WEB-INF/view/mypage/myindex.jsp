...
<body>
	...
	<div class="mypage_list">
		<a href="orderlistpage.store?status=receive" class="list_1"> <span
			class="list_2" id="my_bb_b">${count_receive}</span>
			<div>
				<span class="list_3" id="my_ll_b">주문 접수</span> <span
					class="next_img"></span>
			</div>
		</a> <a href="orderlistpage.store?status=in-shipping" class="list_1">
			<span class="list_2" id="my_bb_b">${count_inshipping}</span>
			<div>
				<span class="list_3" id="my_ll_b">배송 중</span> <span class="next_img"></span>
			</div>
		</a> <a href="orderlistpage.store?status=re-order" class="list_1"> <span
			class="list_2" id="my_bb_b">${count_reorder}</span>
			<div>
				<span class="list_3" id="my_ll_b">취소/반품/교환</span> <span
					class="next_img"></span>
			</div>
		</a> <a href="../mypage/pointlist.store" class="list_4"> <span
			class="list_2" id="my_bb_b"> <c:if test="${count_point>0 }">
					<fmt:formatNumber value="${count_point }" pattern="#,###P" />
				</c:if> <c:if test="${count_point<=0 }">
	...
	<div class="desktop_list">
		<div class="order_list">
			...
			<button class="order_list_b" id="my_ll_b"
				onclick="location.href='orderlistpage.store'">더보기</button>
		</div>
		...
		<c:if test="${count_all>0 }">
			<ul class="admin_list_no_ul" style="border-top: 1px solid black;">
				<li class="order_num" id="my_bb_b" style="font-size: 20px;">주문
					번호</li>
				<li class="item_num_2" id="my_bb_b" style="font-size: 20px;">주문
					상품</li>
				<li class="order_date" id="my_bb_b" style="font-size: 20px;">주문
					일자</li>
			</ul>
			<c:forEach var="order" items="${orderlist }" end="2">
				<div class="order_list1">
					<a class="order_list2"
						href="ordernum.store?orderno=${order.orderno }"
		...
		<div>
			<div class="order_list">
				...
				<button class="order_list_b" id="my_ll_b"
					onclick="location.href='contactpage.store'">더보기</button>
			</div>
			<c:if test="${! empty contactlist}">
				<ul class="admin_list_no_ul" style="border-top: 1px solid black;">
					<!-- 			<li class="order_num" id="my_bb_b" style="font-size: 20px;">주문 번호</li> -->
					<li class="item_num_2" id="my_bb_b" style="font-size: 20px;">문의
						제목</li>
				...
				<c:forEach var="contact" items="${contactlist }" end="2">
					<div class="order_list1">
						...
						<div class="order_list3">
							<div class="order_list3_item">
								<a class="order_list2"> <span> ${contact.q_subject }
								</span>
								</a>
								<%-- <div class="order_list_content">
								<div>
