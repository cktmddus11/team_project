		...
		<ul class="d" id="my_ll_b">
			<c:if test="${orderstate<=5 }">
				<li class="e" <c:if test="${orderstate==0 }">id="my_bb_b"</c:if>>미결제
					<img src="../images/ico_step.png" style="width: 20px; height: 20px">
				</li>
				...
				<ul>
					<c:if test="${orderno_count>1 }">
						<c:forEach var="orderlist" items="${orderlist }">
							<li class="i">
								<div class="j">
									<span class="k"> <img class="l"
										src="../images/product/${orderlist.itemfile1url }" alt="상품사진">
									</span>
									<div class="m">
										<span class="n" id="my_ll_b"> ${orderlist.itemname } </span>
										<div class="o">
											<span id="my_ll_b"><fmt:formatNumber
													value="${orderlist.price }" pattern="#,###원" /></span>
										</div>
									</div>
									<p class="p" id="my_ll_b">${orderlist.quantity }개</p>
									<p class="q" id="my_ll_b">
										<strong id="my_bb_b_1"> <c:choose>
	...
	<div class="r">
		<div class="s">
			...
			<div class="t" style="height: 483px;">
				<div>
					<div class="u">
						<ul class="v">
							<li class="w"><strong class="x1 x2" id="my_bb_b_1">결제수단</strong>
								<div class="y1 y2" id="my_ll_b">
									<c:if test="${selectpay==1 }">신용카드</c:if>
									<c:if test="${selectpay==2 }">무통장입금</c:if>
								</div></li>
							<li class="w"><strong class="x1 x2" id="my_bb_b_1">결제일시</strong>
								<div class="y3 y4 y5" id="my_ll_b">
									<fmt:formatDate value="${datepay }"
										pattern="yyyy-MM-dd HH:mm:ss" />
								</div></li>
						</ul>
						<ul class="z">
							<li class="w"><strong class="x1 x2" id="my_bb_b_1">상품가</strong>
								<span id="my_ll_b"> <c:if test="${orderno_count==1 }">
										<c:set var="total"
											value="${orderlist.price *orderlist.quantity }" />
									</c:if> <c:if test="${orderno_count>1 }">
										<c:forEach var="orderlist" items="${orderlist }">
											<c:set var="sum"
												value="${orderlist.price*orderlist.quantity}" />
										</c:forEach>
										<c:set var="total" value="${sum + sum}" />
									</c:if> <fmt:formatNumber value="${total }" pattern="#,###원" />
							</span></li>
							<!-- <li class="w">
                     <strong class="x1 x2" id="my_ll_b">배송비</strong>
                     <span id="my_ll_b">무료</span>
                  </li> -->
							<li class="w"><strong class="x1 x2" id="my_bb_b_1">포인트</strong>
								<span id="my_ll_b"> <fmt:formatNumber pattern="#,###P"
										value="${usepoint}" />
							</span></li>
							<!-- <li class="w">
                     ...
                     <span id="my_ll_b">원</span>
							...
							<li class="w0 w" id="my_bb_b_2"><strong class="x1 x2"
								id="my_bb_b_2">최종 결제금액</strong> &nbsp;&nbsp; <span
								id="my_bb_b_2"> <c:if test="${orderno_count==1 }">
										<c:set var="total"
											value="${orderlist.price*orderlist.quantity-orderlist.usepoint }" />
		...
		<div class="bb">
			...
			<div class="cc" style="height: 400px;">
				<div>
					<div class="dd-1">
						<strong class="ee" id="my_bb_b_1"> 주문고객 </strong>
						<c:if test="${orderno_count==1 }">
							<dl class="ff">
								<dt class="gg">이름</dt>
