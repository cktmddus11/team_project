			...
			<div class="friends_layer order_layer" style="display: block;">
				<div class="inner_layer">
					...
					<div class="layer_body">
						<form:form modelAttribute="orderlist" method="post" name="f"
							id="nonmemberOrderFrm">
							<input type="hidden" name="_csrf"
								value="4d0ee8fd-7a7e-4965-a362-e36b383ee552">
							<input type="hidden" name="_csrf_header" value="X-CSRF-TOKEN">
							<input type="hidden" id="nonMemberName" name="nonMemberName"
								value="">
							<fieldset>
								...
								<div class="wrap_order">
									<dl class="list_order">
										<dt>
											<label class="lab_order" for="orderno" id="my_ll_b">주문 번호:
												</label>
										</dt>
										<dd>
											<input type="text" id="orderno" name="orderno"
												class="inp_order" value=""
												onkeyup="this.value = this.value.toUpperCase();">
										</dd>
										<dt>
											<label class="lab_order" for="payname" id="my_ll_b">주문자 명:
											<!-- 디자인상 국문은 필드명 '주문자명'이 되어야 함 -->
											</label>
										</dt>
										<dd>
									...
									<!-- display:none 주문번호나 이름 입력값 없을 경우 style="display:block" -->
								</div>
			...
			<script type="text/javascript">
							...
							.on(
									...
									function() {
										...
										if (od_num.length == 0) {
											$(".desc_error")
													.html(
															"&ldquo; 주문번호를 입력해 주세요  &rdquo;");
											$(".desc_error").css("display",
													"block");
											return false;
										}
										if (od_name.length == 0) {
										...
										if (pattern.test(od_num) == true) {
											$(".desc_error")
													.html(
															"&ldquo; 주문번호에 공백이 있습니다. 공백없이 입력해 주세요 &rdquo;");
											$(".desc_error").css("display",
													"block");
											return false;
										}
										...
										$("#nonmemberOrderFrm").attr("action",
												url);
										$("#nonMemberName").val(
												encodeURIComponent(od_name));
										$("#orderno").val(od_num);
										$("#userid").val(od_mail);
				...
				function fn_orderCheck() {
					...
					$.ajax({
						...
						dataType : "json",
						data : {
							orderno : $.trim($("#nonmemberOrderFrm #orderno")
									.val()),
							payname : $.trim($("#nonmemberOrderFrm #payname")
									.val()),
							userid : $.trim($("#nonmemberOrderFrm #userid")
									.val())
						},
						...
						error : function(e) {
							...
							console.log($.trim($("#nonmemberOrderFrm #orderno")
									.val()));
							return false;
						},
