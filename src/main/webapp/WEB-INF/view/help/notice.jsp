...
.avmum {
	...
	height: 34.6667vw;
	background:
		url(https://t1.kakaocdn.net/friends/new_store/2.0/mobile/banner_faq.png)
   ...
   function listdo(page){
	...
	<div class="avmum" style="height: 13.7vw; background-color: #fdd33a;">
		<span class="category_header_span" id="boldfont_white_he"
			style="font-weight: bold;">고객센터</span>
	</div>
	<div class="title jCUDnA">
		<div class="header gHsTgc">
			<button class="notice_button tab ewldGg" id="tab1 my_bb_b"
				type="button" onclick="disp_div('notice', 'tab1')">공지사항</button>
		...
		<form name="selectform" action="notice.store" method="post">
			<input type="hidden" name="tab" value=""> <input
				type="hidden" name="selectvalue" value="">
         ...
         ${fn:trim(param.selectvalue)}
				...
				<div class="header__Select kDPxRN">
					<ul id="box my_ll_b"
						class="select-modal__OptionWrap-ow3cz8-0 drWdwN" type="text">
					...
					<!--  전체라는 값을 읽어서 전체면 1전송하고그런식으로 해야할듯 -->
				</div>
	...
	<c:if test="${qnalistcount == 0}">
		<div class="admin_list fkzQYP"
			style="padding-left: 30vw; padding-top: 10vw; padding-bottom: 10vw;">
			<!-- count == 0 -->
		...
		<c:forEach var="qnalist" items="${qnalist}" varStatus="stat">
			<div class="list__ItemWrapper-sc-15wis6g-0 fkzQYP">
				<div class="list__TitleBlock-sc-15wis6g-3 gxwoaN">
					...
					<div class="faq_title glsXdQ"
						onclick="replyshow(2, 'faqreplynum${stat.index}', this)">${qnalist.q_subject}</div>
				</div>
			</div>
