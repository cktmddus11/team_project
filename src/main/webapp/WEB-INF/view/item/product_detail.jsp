			...
			<div class="detail-header__Detail-vk2zi5-0 eoPXCl">
				...
				<div class="detail-header__PriceWrap-vk2zi5-13 eYsRXP">
					...
					<div class="hearticon p-t-25">
						...
						<a href="#" class="" id="heart"> <c:if
								test='${item.wishcheck==1}'>
								<img class="icon-heart1" src="../images/icons/icon-heart-01.png"
									alt="ICON" style="display: none;">
								<img class="icon-heart2 select"
									src="../images/icons/icon-heart-02.png" alt="ICON" style="">
							</c:if> <c:if test='${item.wishcheck==0}'>
								<img class="icon-heart1" src="../images/icons/icon-heart-01.png"
									alt="ICON" style="">
								<img class="icon-heart2" src="../images/icons/icon-heart-02.png"
									alt="ICON" style="display: none;">
							</c:if>
						</a>
					</div>
			...
			<div class="review__Wrap-yivm8l-0 itdkzt">
				...
				<c:if test='${check == 1}'>
					<button class="review__NewReview-yivm8l-1 fQZhjJ"
						onclick="location.href='review.store?no=${param.no}'">리뷰를
						남겨주세요</button>
				</c:if>
				<c:if test='${check != 1}'>
					<button class="review__NewReview-yivm8l-1 fQZhjJ"
						onclick="writecheck()">리뷰를 남겨주세요</button>
				</c:if>
				<script>
					function writecheck(){
				...
				<c:if test='${!empty item.reviewlist}'>
					<div class="list__Wrap-sc-7txpwb-0 jSEjVi">
						...
						<ul class="list__Ul-sc-7txpwb-1 gxJvJD">
							<c:forEach var="reviewlist" items="${item.reviewlist}"
								varStatus="stat">
								...
								<li class="list__Li-sc-7txpwb-2 jeZgoZ">
									...
									<div class="list__StarWrap-sc-7txpwb-8 hTSHGu">
										<fmt:parseNumber var="reavg" value="${reviewlist.itemavg}"
											integerOnly="true" />
									...
									<div class="list__RecommendWrap-sc-7txpwb-12 hURYlD">
										<c:if test='${!empty reviewlist.uplist }'>
											<c:forEach var="uplist" items="${reviewlist.uplist}">
												<c:choose>
													<c:when
														test='${sessionScope.loginUser.userid eq uplist.userid}'>
														<button
															class="list__RecommendReviewButton-sc-7txpwb-6 hWuFcD"
															id="recommend${stat.index}"
															onclick="recommend(${stat.index}, ${reviewlist.reviewno},  '${reviewlist.userid}')">
															도움돼요 <span id="reviewcount">${reviewlist.upcount }</span><span>명</span>
															<!--  누르면 추천 증가하게 ajax으로 -->
															<!--  hWuFcD 누르면 이걸로       jOEcHV기본-->
														</button>
													</c:when>
													<c:when
														test='${sessionScope.loginUser.userid ne uplist.userid}'>
														<button
															class="list__RecommendReviewButton-sc-7txpwb-6 jOEcHV"
															id="recommend${stat.index}"
															onclick="recommend(${stat.index}, ${reviewlist.reviewno},  '${reviewlist.userid}')">
															도움돼요 <span id="reviewcount">${reviewlist.upcount }</span><span>명</span>
															<!--  누르면 추천 증가하게 ajax으로 -->
										...
										<p class="list__RecommendCountPc-sc-7txpwb-13 jxBTIN">
											<span id="reviewcount1">${reviewlist.upcount}</span>명에게
											도움이됐어요
										</p>
			...
			<div class="bottom-bar__BottomWrap-sc-11z5au4-1 hZwLyu">
				<div class="no-option__Wrap-z0m2t2-1 jUxlri">
					...
					<div class="no-option__Price-z0m2t2-0 hCwYWR">
						<span class="no-option__Label-z0m2t2-2 kMgbXi">총 상품금액</span><span
							id="allprice"> <fmt:formatNumber value="${item.price}"
								type="currency" currencySymbol="" /></span>원
					</div>
				</div>
				...
				<div class="bottom-bar__Wrap-sc-11z5au4-0 eyQXYE">
					<form method="get" action="../order/order.store" name="f">
						...
						<input type="hidden" name="oquantity" value=""> <input
							type="hidden" name="oprice" value="">
					</form>
					<button type="button"
						class="direct-purchase__Purchase-sc-1jgpcp-0 iipgZi"
						onclick="javasccript:document.f.submit()">바로구매</button>
					<!--   js-show-modal2일단지움 -->
					<c:if test="${item.cartcheck == 1}">
						<button type="button"
							class="sc-bwzfXH hnazCY button-oulv2u-0 eGyuxj" image="cart">
							장바구니
							<!--   js-show-modal2일단지움 -->
							담기
						</button>
					...
					<c:if test="${item.cartcheck == 0}">
						<button type="button"
							class="sc-bwzfXH hnazCY button-oulv2u-0 qSzXk" image="cart">
							장바구니
							<!--   js-show-modal2일단지움 -->
							담기
						</button>
