...
$(function(){
   if(${param.category == 1}){
	...
	<div class="category_header_div" style="height: 12.7vw">
		<span class="category_header_span" id="boldfont_white_he"
			style="font-weight: bold;"> <c:if
				test='${param.category == 1}'>토이</c:if> <c:if
				test='${param.category == 2}'>의류</c:if> <c:if
				test='${param.category == 3}'>생활테크</c:if> <c:if
				test='${character==1 && empty param.category}'>라이언</c:if> <c:if
				test='${character==2 && empty param.category}'>어피치</c:if> <c:if
				test='${empty param.category && empty character}'>전체</c:if>
		...
		<c:if test='${!empty param.category}'>
			<div class="sub_cate_in2">
				<div style="border-bottom: 1px solid rgba(0, 0, 0, 0.2)">
					<button class="sub_cate_btn2"
						onclick="location.href='../item/list.store?category=${param.category}'">전체</button>
					<button class="sub_cate_btn1"
						onclick="location.href='../item/list.store?category=${param.category}&subcategory=2'">
						<c:if test='${param.category==1}'>중형인형</c:if>
						<c:if test='${param.category==2}'>남성</c:if>
						<c:if test='${param.category==3}'>에어팟 악세서리</c:if>
					</button>
					<button class="sub_cate_btn3"
						onclick="location.href='../item/list.store?category=${param.category}&subcategory=1'">
						<c:if test='${param.category==1}'>미니인형</c:if>
						<c:if test='${param.category==2}'>여성</c:if>
						<c:if test='${param.category==3}'>에어팟 케이스</c:if>
					</button>
				</div>
			</div>
		</c:if>
	...
	<form name="selectform" action="list.store" method="post"
		...
		<div style="display: inline-block; position: relative; padding-top: 20px; float: right;">
			<div class="dropdown" style="width: 200px; height: 50px;">
				<div class="select" id="select2">
					<span><c:if test='${empty select_2}'>신상품순</c:if> <c:if
							test='${!empty select_2}'>${select_2}</c:if></span> <i
						class="fa fa-chevron-up"
						style="margin-right: 0px; margin-top: 0px;"></i>
				</div>
		...
		<div style="display: inline-block; position: relative; padding-top: 20px; float: right;">
			<div class="dropdown" style="width: 200px; height: 50px;">
				<div class="select" id="select1">
					<span><c:if test='${empty select_1}'>전체</c:if> <c:if
							test='${!empty select_1}'>${select_1}</c:if> </span> <i
						class="fa fa-chevron-up"
						style="margin-right: 0px; margin-top: 0px;"></i>
				</div>
				...
				<div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item women">
					...
					<div class="block2">
						<div class="block2-pic hov-img0">
							<img src="../images/product/${itemlist.itemfile1url}"
								alt="IMG-PRODUCT">
						</div>
						<div class="block2-txt flex-w flex-t p-t-14">
							<div class="block2-txt-child1 flex-col-l " style="width: 70%;">
								<a href="product_detail.store?no=${itemlist.itemnum}"
									...
									id="my_ll_z"> ${itemlist.itemname}</a> <span
									class="stext-105 cl3" id="my_l_b"> <fmt:formatNumber
										value="${itemlist.price}" type="currency" currencySymbol="" />원
								</span>
							</div>


							<div class="wish_check" itemtype="new">
								<input id="item${stat.index}" type="checkbox"
									data-toggle="tooltip"
									onclick="checkclick(${itemlist.pickno}, ${itemlist.itemnum},  'item${stat.index}')"
									data-placement="top" title="찜하기"
									<c:if test="${itemlist.pickno!=0}">checked="checked"</c:if>>
								<label for="item${stat.index}"></label> <input type="hidden"
									name="pickno" value="${itemlist.pickno}">
							</div>
						</div>
					</div>
				</div>
