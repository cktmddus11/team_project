...
.bHdxYH {
	...
	text-indent: -999em;
	font-size: 1px;
	background: url(../images/product/btn_remove.png) center center/3.73333vw
		no-repeat;
}
...
<body>
	<div id="kakaoWrap">
		...
		<c:if test="${(!empty cart && (fn:length(cart.itemSetList)!=0)) || !empty cartdb}">
			<c:if test="${empty cartdb && fn:length(cart.itemSetList) > 0}">
				<div class="check_all">
					<div class="checks">
						<input type="checkbox" id="ex_chk" name="allchk"
							onchange="allchkbox(this)" checked> <label for="ex_chk"
							style="margin-bottom: 20px;">전체</label>
					</div>
							...
							<div class="order_list1">
								<div class="order_list3">
									<div class="order_list3_item">
										...
										<a class="order_list3_img"> <span
											onclick="location.href='../item/product_detail.store?no=${cart.item.itemnum}'"><img
												style="width: 100%; height: auto;"
												src="../images/product/${cart.item.itemfile1url}" alt="상품명">
										</span>
										</a>
										<div class="order_list_content">
											<div class="BTcrB">
												<div class="item_name" id="my_ll_b">${cart.item.itemname}</div>
												<button class="basket-item__RemoveButton-sc-1bhrte-7 bHdxYH"
													onclick="clickdelete(${cart.item.itemnum}, '${cart.item.itemname}', ${stat.index})"></button>
								...
								<div class="order_list4_1"
									...
									<div>
										<input type="number" value="${cart.quantity}"
											style="border-bottom: 1px #000 solid; width: 115px; text-align: -webkit-center;">
									</div>
								</div>
						...
						<div class="count" style="color: #000;">
							<div class="count_block">
								<span class="all_order_price"
									style="padding-left: 500px; padding-top: 18px;">총 주문 금액</span>
								<span class="price"
									style="padding-top: 12px; padding-right: 15px;"> <fmt:formatNumber
										value="${tot}" type="currency" currencySymbol="" /> 원
								</span>
							</div>
							<div class="count_block">
			...
			<c:if test="${empty cart}">
				...
				<div class="check_all">
					<div class="checks">
						<input type="checkbox" id="ex_chk" name="allchk"
							onchange="allchkbox(this)"> <label for="ex_chk"
							style="margin-bottom: 20px;">전체</label>
					</div>
					<div class="checks_delete">
						<span class="selects">1개 선택&nbsp;</span>
									...
									<div class="order_list3_item">
										...
										<div class="order_list_content">
											<div class="BTcrB">
												<div class="item_name" id="my_ll_b">${cart.item.itemname}</div>
												<button class="basket-item__RemoveButton-sc-1bhrte-7 bHdxYH"
													onclick="clickdelete(${cart.item.itemnum}, '${cart.item.itemname}', 0)"></button>
												<input type="hidden" name="itemnum"
													value="${cart.item.itemnum}">
											</div>
										</div>
			...
			<div class="laCToX" style="margin: auto;">
				<input type="button" value="주문하기" id="order" style="margin: auto;"
					onclick="location.href='../order/order.store'">
			</div>
	...
	<c:if
		test="${(empty cart || fn:length(cart.itemSetList) == 0) && empty cartdb}">
		<div class="empty_basket">
			<div class="empty_basket_msg">
				아직 관심 상품이 없네요! <br> 귀여운 프렌즈 상품을 추천드릴게요 <br>
				<br>
			</div>
			<a class="pop_list_link" href="../item/list.store"> <span
				class="pop_link">인기 상품 보기</span>
			</a>
		</div>
	</c:if>
	<script>
